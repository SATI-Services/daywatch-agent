...
State is **partitioned by author so writes never collide; everyone reads everything.** **`<author>` is always the person** (from git identity), whichever harness drove the work — agents stamp their harness *inside* the files: the `**Agent:** <harness> (<model>)` field in dated logs and a per-agent `## <harness>` section in `status/<author>.md`, each agent editing only its own section (concurrent agents of one person → Track A2 worktrees, so disjoint sections merge cleanly). **Never leave the model implied by the harness:** the stamp names the harness *and its model* (`**Agent:** <harness> (<model>)`, e.g. `kimi (deepseek/deepseek-v4.1-flash)`). See [`docs/sessions/README.md`](docs/sessions/README.md).
